...
module.exports = class Ecard {
  ...
  //获取token
  async getToken(PhoneNO, cache = true) {
    ...
    if (token) {
      ...
      if (cache) {
        await redisClient.setex(`ecard_Token${PhoneNO}`, 1 * 60 * 60, token);
  ...
  getTimeStr() {
    ...
    const TimeStr = `${parseInt(TimeStamp / 1000, 10)}`;
    return TimeStr;
  }

  async fetch(json) {
    const token = await this.getToken(json.PhoneNO, json.MethodName !== 'UserRegister');
    ...
    console.log(`cost ${new Date() - start}ms`, options, data);
    if (data.RtnCode !== 0) {
