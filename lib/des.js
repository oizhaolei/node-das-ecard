...
const encrypt = (key, input, iv) => {
  console.log('des.encrypt', key, input, iv);
  const genKey = genkey(key, 0, 24);
  return btoa(des(genKey.key, input, 1, 1, des3iv(iv), 1));
};
////3DES解密，CBC/PKCS5Padding
const decrypt = (key, input, iv) => {
  const genKey = genkey(key, 0, 24);
  return des(genKey.key, atob(input), 0, 1, des3iv(iv), 1);
};
